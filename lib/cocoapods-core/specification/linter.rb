    ...
    class Linter
      ...
      def lint
        ...
        if spec
          ...
          results.error "[spec] The specification defined in `#{file}` could" \
          ' not be loaded.' \
            "\n\n#{@raise_message}"
        end
      ...
      #
      def errors
        @errors ||= results.results.select { |r| r.type == :error }
      end

      # @return [Array<Result>] all the warnings generated by the Linter.
      #
      def warnings
        @warnings ||= results.results.select { |r| r.type == :warning }
      end
        ...
        attributes.each do |attr|
          ...
          unless value && (!value.respond_to?(:empty?) || !value.empty?)
            if attr.name == :license
              results.warning('[attributes] Missing required attribute' \
              "`#{attr.name}`.")
            else
              results.error('[attributes] Missing required attribute' \
               "`#{attr.name}`.")
            end
          end
        end
      ...
      def _validate_name(_n)
        if spec.name && file
          ...
          unless names_match
            results.error '[name] The name of the spec should match the ' \
                           'name of the file.'
          end

          if spec.root.name =~ /\s/
            results.error '[name] The name of a spec should not contain ' \
                           'whitespace.'
          end

          if spec.root.name[0, 1] == '.'
            results.error '[name] The name of a spec should not begin' \
            ' with a period.'
          end
        end
      end
      ...
      def _validate_version(v)
        if v.to_s.empty?
          results.error '[version] A version is required.'
        elsif v <= Version::ZERO
          results.error '[version] The version of the spec should be' \
          ' higher than 0.'
        end
      end
      ...
      def _validate_summary(s)
        if s.length > 140
          results.warning '[summary] The summary should be a short ' \
            'version of `description` (max 140 characters).'
        end
        if s =~ /A short description of/
          results.warning '[summary] The summary is not meaningful.'
        end
      end
      ...
      def _validate_description(d)
        if d =~ /An optional longer description of/
          results.warning '[description] The description is not meaningful.'
        end
        if d == spec.summary
          results.warning '[description] The description is equal to' \
           ' the summary.'
        end
        if d.length < spec.summary.length
          results.warning '[description] The description is shorter ' \
          'than the summary.'
        end
      end
      ...
      def _validate_homepage(h)
        if h =~ %r{http://EXAMPLE}
          results.warning '[homepage] The homepage has not been updated' \
           ' from default'
        end
      end
      ...
      def _validate_frameworks(frameworks)
        if frameworks_invalid?(frameworks)
          results.error '[frameworks] A framework should only be' \
      ...
      def _validate_weak_frameworks(frameworks)
        if frameworks_invalid?(frameworks)
          results.error '[weak_frameworks] A weak framework should only be' \
          ' specified by its name'
        end
      end
      ...
      def _validate_libraries(libs)
        libs.each do |lib|
          ...
          if lib.end_with?('.a') || lib.end_with?('.dylib')
            results.error '[libraries] Libraries should not include the' \
            ' extension ' \
            "(`#{lib}`)"
          end

          if lib.start_with?('lib')
            results.error '[libraries] Libraries should omit the `lib`' \
            ' prefix ' \
            " (`#{lib}`)"
          end

          if lib.include?(',')
            results.error '[libraries] Libraries should not include comas ' \
            "(`#{lib}`)"
          end
        end
      end
      ...
      def _validate_license(l)
        ...
        file = l[:file]
        if type.nil?
          results.warning '[license] Missing license type.'
        end
        if type && type.gsub(' ', '').gsub("\n", '').empty?
          results.warning '[license] Invalid license type.'
        end
        if type && type =~ /\(example\)/
          results.error '[license] Sample license type.'
        end
      ...
      def _validate_source(s)
        if git = s[:git]
          ...
          if git =~ %r{http://EXAMPLE}
            results.error '[source] The Git source still contains the ' \
            'example URL.'
          end
          if commit && commit.downcase =~ /head/
            results.error '[source] The commit of a Git source cannot be' \
            ' `HEAD`.'
          end
          if tag && !tag.to_s.include?(version)
            results.warning '[source] The version should be included in' \
             ' the Git tag.'
          end
          if tag.nil?
            @results.warning '[source] Git sources should specify a tag.'
          end
        end
      ...
      def perform_github_source_checks(s)
        ...
        if git = s[:git]
          ...
          if git_uri.host == 'www.github.com'
            results.warning '[github_sources] Github repositories should not' \
            ' use `www` in URL.'
          end
          if git_uri.host == 'github.com' || git_uri.host == 'gist.github.com'
            unless git.end_with?('.git')
              results.warning '[github_sources] Github repositories should' \
              ' end in `.git`.'
            end
      ...
      def _validate_social_media_url(s)
        if s =~ %r{https://twitter.com/EXAMPLE}
          results.warning '[social_media_url] The social media URL has not' \
          'been updated from default'
        end
      end
      ...
      def _validate_compiler_flags(flags)
        if flags.join(' ').split(' ').any? { |flag| flag.start_with?('-Wno') }
          results.warning '[compiler_flags] Warnings must not be disabled' \
          '(`-Wno compiler` flags).'
        end
      end
