...
module Pod
  describe Source::Validator do
    describe 'In general' do
      ...
      # it "checks the path of the specification" do
      ...
      it "rejects a Git based specification without a tag not marked as 0.0.1" do
        source = { :git => 'http://banana-corp.local/banana-lib.git', :commit => 'SHA' }
        @spec.stubs(:name).returns('MyPod')
        @spec.stubs(:source).returns(source)
        @spec.stubs(:version).returns(Version.new('1.0'))
        check_error('Untagged Git repositories should be versioned as 0.0.1')
      end

      it "checks if there an attempt to change the commit of an untagged version" do
        @validator.stubs(:check_if_untagged_version_is_acceptable)
